...
class EHeistLGame : public BasicAbstractGame {
  public:
    ...
    void game_step() override {
        ...
        for (int i = 0; i < entities.size(); i++){
            ...
            if (ent->type == LOCKED_DOOR){
                data[3] = ent->x;
                data[4] = ent->y;
                data[5] = ent->image_theme;
            }
        }

        // data[3] = entities[key_count]->x;
        // data[4] = entities[key_count]->y;
        // data[5] = entities[key_count]->image_theme;

        step_data.reward += get_reward(data[0], data[1], data[3], data[4]);

        // for (int i = 0; i < key_count; i++){
        //     data[(i+1)*3 + 0] = entities[i+1]->x;
        //     data[(i+1)*3 + 1] = entities[i+1]->y;
        //     data[(i+1)*3 + 2] = entities[i+1]->image_theme;
        // }
...
REGISTER_GAME(NAME, EHeistLGame);
