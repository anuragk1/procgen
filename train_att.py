...
from utils.policies import preprocess_obs, split_obs

exp_name = os.path.basename(__file__).rstrip(".py")
gym_id = "bigfishr"
learning_rate = 2.5e-4
seed = 1
total_timesteps = int(1e8*5)
...
for update in range(1, num_updates+1):
    ...
    for step in range(0, num_steps):
        ...
        # obs_split = split_obs(processed_obs[step])
        # start_time_loop = time.time()
        # with torch.no_grad():
        # for i in range(len(action)):
            # print(processed_obs[step][i])
            # if action[i] == 0:
                # expert_action, _, _ = agent_eat.get_action(processed_obs[step][i])
                # expert_action = agent_eat.get_action(processed_obs[step][i])
                # print(f"expert_action : {expert_action}")
                # expert_actions[i] = expert_action

            
            # elif action[i] == 1:
                # expert_action, _, _ = agent_dodge.get_action(processed_obs[step][i])
                # expert_action = agent_dodge.get_action(processed_obs[step][i])
                # print(f"expert_action : {expert_action}")
                # expert_actions[i] = expert_action

        # print(f"agent_eat.get_action(processed_obs[step]) : {agent_eat.get_action(processed_obs[step])}")
        # print(f"time per loop1 : {start_time_loop - time.time()}")
        # start_time_loop = time.time()
        ...
        # print(f"time per loop2 : {start_time_loop - time.time()}")
